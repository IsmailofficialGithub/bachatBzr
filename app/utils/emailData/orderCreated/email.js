export const OrderCreateHTML = (orderId, products = [], totalPrice) => {
  const productsHtml = products.map(
    (item) => `<li>${item.name} - PKR ${item.price}</li>`
  ).join("");
  const trackUrl = `${process.env.NEXT_PUBLIC_API_URL}/track#${orderId}`;
  return `
      <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
      <div class="container">
        ...
        <p>This is your order ID to track your order.</p>
      ...
      Track My Order
        ...
        <p style="margin-top: 30px;">We appreciate your business and hope to serve you again soon!</p>
